...
void MBR::solve()
{
  ...
  if (this->feature == game->bm_n_vars) {
    // There is only a single configuration left. The VPG only contains edges admitting this configurations, so it
    // essentially is a parity game which we now solve.
    ...
    if (metric_output) {
      ...
      cout << "Assisted leaf " << (*this->measured->value)[0] + (*this->measured->value)[1] << " with time "
           << (*this->measured->value)[2] << endl;
    }
    else {
      fpite.solve();
    }
  ...
  if (metric_output) {
    ...
    auto end = std::chrono::high_resolution_clock::now();
    (*this->measured->value)[2] = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    //            if(std::count_if(W0.begin(), W0.end(), [](bool b){return b;}) - std::count_if(P0->begin(), P0->end(),
    //            [](bool b){return b;}) < 3000)
    //        if(this->feature != 0)
    ...
    cout << "Assisted node " << (*this->measured->value)[0] + (*this->measured->value)[1] << " with time "
         << (*this->measured->value)[2] << endl;
  }
  else {
    fpite0->solve();
  }
  // Vertices won by player 0 in this game are won by player 0 for all configurations. Therefore we also now these are
  // never won by player 1 in pessimistic game 1
  *P0 = W0;
  ...
  if (metric_output) {
    ...
    //            if(std::count_if(VP1->begin(), VP1->end(), [](bool b){return b;}) - std::count_if(W0.begin(),
    //            W0.end(), [](bool b){return b;}) < 3000)
    //        if(this->feature != 0)
  ...
  *VP1 = W0;
  ...
  if (this->solvelocal) {
    ...
    done = (*P0)[game->reindexedNew[game->findVertexWinningFor0()]]
           || !(*VP1)[game->reindexedNew[game->findVertexWinningFor0()]];
  }
  else {
...
void MBR::createSubGames(ConfSet* confP,
    vector<std::tuple<int, int>>* subgame_out,
