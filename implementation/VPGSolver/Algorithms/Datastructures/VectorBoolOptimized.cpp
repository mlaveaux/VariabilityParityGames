...
{
  int i = start;
  // start probably lays somewhere within an integer. First we copy the remaining part of that integer so we end up
  // exactly at some index divisably by 64
  ...
  if (i != start + n) {
    // still work to do, copy complete integers for integers such that all boolean values represented by them are in the
    // range of start to start+n
