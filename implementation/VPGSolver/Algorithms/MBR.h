...
protected:
  ...
  /**
   * Create new edge relation only containing configurations in confP, that is edges that do not contain any
   * configuration in confP are removed from the edge relation.
   ...
   */
  void
  createSubGames(ConfSet* confP, vector<std::tuple<int, int>>* subgame_out, vector<std::tuple<int, int>>* subgame_in);
  /**
   ...
   */
  void createPessimisticGames(vector<std::tuple<int, int>>* pessimistic_out0,
