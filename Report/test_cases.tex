...
We use $\lambda$ to denote the average relative size of guard sets in VPGs. We create random games with a specific $\lambda$ by using a probabilistic distribution ranging from $0$ to $1$ to determine the size of every guard set. Such a distribution must have a mean equal to $\lambda$. We consider two distributions:
\begin{itemize}
