...
\begin{example}
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}[->]
			...
			\path (v6) edge node[above]{$\{c_0,c_1\}$} (v5);
...
\begin{example}
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}[->,scale=0.9]
			...
			\path (s1<b>T) edge node[left]{$\{c_0,c_1\}$} (l0);
			\path (l0) edge[loop right] node[right]{$\mathfrak{C}$} (l0);
			\path (s1<b>T) edge node[fill=white]{$\{c_2\}$} (s1T);
			...
			\path(s1[a]muxphi) edge[bend right] node[right]{$\{c_1,c_2\}$} (s1muxphi);
