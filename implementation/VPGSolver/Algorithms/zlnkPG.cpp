...
{
  vector<int> successorsnotinbigA(game->n_nodes); // count the number of successors not attracted
  vector<bool> countinitialized(game->n_nodes);   // indicate for every vertex if successorsnotinbigA is initialized
  queue<int> qq;
  ...
  while (!qq.empty()) {
    ...
    for (auto& i : game->in_edges[vii]) {
      ...
      // vertex vii has been placed into the attractor set, so vi has at least one successor that is attracted, namely
      // vii if vertex vi is owned by player "player" then we can attract vi
      bool attracted = true;
  ...
  if (!WOpbigV->any()) {
    ...
    if (inSolveLocal(1 - player) && (*WOpbigV)[0]) {
      ...
      *bigA = *WOpbigV;
      ...
      // Attract bigA (which is now contains the vertices won by player "1- player" in the subgame) for player "1 -
      // player"
