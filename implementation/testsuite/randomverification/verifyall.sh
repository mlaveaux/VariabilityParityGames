...
./verify_single_local.sh VPGSolver_bdd_single | grep -v Start
...
echo "Verify compressed"
./verify.sh VPGSolver_bdd v| grep -v Start
echo "Verify compressed single FPIte"
./verify.sh VPGSolver_bdd F v|grep -v Start
