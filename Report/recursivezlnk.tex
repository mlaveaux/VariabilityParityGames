...
Given $X^\lambda =_\lambda X$ and $Y^\lambda =_\lambda Y$, then clearly $X^\lambda \cup Y^\lambda =_\lambda X \cup Y$. We also define the subset or equal operator for two function $X^\lambda : \hat{V} \rightarrow 2^\mathfrak{C}$ and $Y^\lambda : \hat{V} \rightarrow 2^\mathfrak{C}$ as
...
\begin{example}
	Consider unified parity game $G = (V,\hat{V}_0,\hat{V}_1,E,\hat{\Omega})$ originating from a VPG with configuration $\mathfrak{C} = \{c_1,c_2,c_3\}$ represented function-wise in Figure \ref{fig:upg}. We depict every $(v,w)$ for which $E(v,w) \neq \emptyset$ with an edge annotated by the set $E(v,w)$. All the origin vertices are depicted and for every origin vertex $\hat{v}$ we annotate the square or diamond with a label $\hat{v}\ |\ C$ where $C = V(\hat{v})$.
	\begin{figure}[h]
		\centering
		\begin{tikzpicture}[->]
			\tikzstyle{even} = [diamond,draw,minimum size=0.75cm]
...
\[U_1 = \{\hat{v}_1 \mapsto \mathfrak{C}, \hat{v}_2 \mapsto \mathfrak{C}, \hat{v}_3 \mapsto \mathfrak{C}, \hat{v}_4 \mapsto \emptyset, \hat{v}_5 \mapsto \{c_2\}, \hat{v}_6 \mapsto \emptyset, \hat{v}_7 \mapsto \emptyset\}\]
Note that $\hat{v}_5$ can be attracted for configuration $\{c_2\}$ because $\mathfrak{C} \backslash E(\hat{v}_5,\hat{v}_7) = \{c_2\}$, $U_0(\hat{v}_3) = \mathfrak{C}$ and for any other origin vertex $\hat{v}$ we have $\mathfrak{C} \backslash E(\hat{v}_5,\hat{v}) = \mathfrak{C}$.

In the next iterations we find
\[U_1 = \{\hat{v}_1 \mapsto \mathfrak{C}, \hat{v}_2 \mapsto \mathfrak{C}, \hat{v}_3 \mapsto \mathfrak{C}, \hat{v}_4 \mapsto \{c_1,c_2\}, \hat{v}_5 \mapsto \{c_2\}, \hat{v}_6 \mapsto \{c_1,c_3\}, \hat{v}_7 \mapsto \emptyset\}\]
\[U_2 = \{\hat{v}_1 \mapsto \mathfrak{C}, \hat{v}_2 \mapsto \mathfrak{C}, \hat{v}_3 \mapsto \mathfrak{C}, \hat{v}_4 \mapsto \{c_1,c_2\}, \hat{v}_5 \mapsto \{c_2\}, \hat{v}_6 \mapsto \{c_1,c_3\}, \hat{v}_7 \mapsto \{c_1\}\}\]
\[U_3 = \{\hat{v}_1 \mapsto \mathfrak{C}, \hat{v}_2 \mapsto \mathfrak{C}, \hat{v}_3 \mapsto \mathfrak{C}, \hat{v}_4 \mapsto \{c_1,c_2\}, \hat{v}_5 \mapsto \{c_1,c_2\}, \hat{v}_6 \mapsto \{c_1,c_3\}, \hat{v}_7 \mapsto \{c_1\}\}\]
...
\begin{figure}[h]
	\centering\begin{tikzpicture}[->]
		...
		\node[odd, label={north:$\hat{v}_5\ |\ \{c_3\}$}] (v5) at (23,17) {2};
		\node[odd, label={north:$\hat{v}_6\ |\ \{c_2\}$}] (v6) at (26,17) {3};
		
		\node[odd, label={south:$\hat{v} _7\ |\ \{c_2,c_3\}$}] (v7) at (23,14) {2};
		
		\path (v4) edge node[fill=white]{$\{c_3\}$} (v5);
		
		
		\path (v7) edge node[fill=white]{$\{c_3\}$} (v4);
		\path (v5) edge node[left,near start]{$\{c_3\}$} (v7);
		\path (v7) edge[bend left] node[fill=white]{$\{c_2\}$} (v6);
		\path (v6) edge[bend left] node[fill=white]{$\{c_2\}$} (v7);
	\end{tikzpicture}
...
We say that an origin vertex $\hat{v}$ and configuration $c$ can be attracted to $A^\lambda \subseteq V^\lambda$ when it meets the criteria required to be added to the attractor set. Formally, using Definition \ref{def_Uattr}, $c$ and $\hat{v}$ can be attracted to $A^\lambda$ when 
\[
...
We consider the running time for solving VPG $G = (V,V_0,V_1,E,\Omega,\mathfrak{C},\theta)$ independently and collectively using the different types of representations. We use $n$ to denote the number of vertices, $e$ the number of edges, $d$ the number of distinct priorities and $c$ to indicate the number of configurations.
