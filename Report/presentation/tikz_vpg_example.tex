...
\if\playconf2\def\opacitycone{0.3}\else\def\opacitycone{1}\fi%
\if\playconf1\def\opacityctwo{0.3}\else\def\opacityctwo{1}\fi%
\begin{tikzpicture}[->,scale=0.9, every node/.style={scale=0.8}]
	...
	\node[even,label=east:$v_4$,fill=\fillvd,label=center:\if\showprio1{4}\fi] (v4) at (2,0) {};
	\node[odd,label=east:$v_5$,fill=\fillve,label=center:\if\showprio1{1}\fi] (v5) at (4,1) {};
	\node (a) at (4.5,2) {$\mathfrak{C} = \{c_1,c_2\}$};
	
	\path (v4) edge node[fill=bgTUE]{$\mathfrak{C}$} (v5) ;
	\path (v3) edge [bend right] node[fill=bgTUE]{$\mathfrak{C}$} (v1) ;
	\path (v1) edge [bend right] node[fill=bgTUE]{$\mathfrak{C}$} (v3) ;
	\path (v3) edge node[fill=bgTUE]{$\mathfrak{C}$} (v4) ;
	\path[opacity=\opacityctwo] (v2) edge node[fill=bgTUE]{$\{c_2\}$} (v4) ;
	\path[opacity=\opacitycone] (v2) edge node[fill=bgTUE]{$\{c_1\}$} (v1) ;
	\path (v5) edge node[fill=bgTUE]{$\mathfrak{C}$} (v2) ;
\end{tikzpicture}